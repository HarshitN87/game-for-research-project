...
export default class CharacterController extends Component{
    ...
    get IsPlayerInHitbox(){
        this.tempVec.copy(this.player.Position).sub(this.model.position);
        const distSq = this.tempVec.lengthSq();
        return this.hitbox.overlapping || distSq <= (this.attackDistance + 0.3) * (this.attackDistance + 0.3);
    }
    ...
    TakeHit = msg => {
        ...
        if(this.health == 0){
            if(!this.defeated){
                ...
                // Remove physics collisions
                const collision = this.GetComponent('CharacterCollision');
                if (collision) {
                    collision.RemoveCollisions();
                }
                const attackTrigger = this.GetComponent('AttackTrigger');
                if (attackTrigger) {
                    attackTrigger.RemoveTrigger();
                }

                window.dispatchEvent(new CustomEvent('fps-enemy-defeated', { detail: { name: this.parent.Name } }));
            }
    ...
    Update(t){
        ...
        // Decay red hit flash emissive intensity
        if (this.meshes) {
            this.meshes.forEach(mesh => {
                if (mesh.material) {
                    const materials = Array.isArray(mesh.material) ? mesh.material : [mesh.material];
                    materials.forEach(mat => {
                        if (mat.emissive && mat.emissiveIntensity > 0) {
                            mat.emissiveIntensity = Math.max(0, mat.emissiveIntensity - t * 6.0);
                            if (mat.emissiveIntensity === 0 && mat.originalEmissive) {
        ...
        // Slowly sink corpse into the ground after 4 seconds of defeat
        if (this.defeated) {
            if (this.sinkTimer === undefined) {
                this.sinkTimer = 0.0;
            }
            this.sinkTimer += t;
            if (this.sinkTimer > 4.0) {
                this.model.position.y -= t * 0.4;
                if (this.model.position.y < -3.0) {
