...
export default class CharacterCollision extends Component{
    ...
    Initialize(){
        ...
        Object.keys(this.collisions).forEach(key=>{
            ...
            const scale = 1.7;
            const shape = new Ammo.btCapsuleShape(collision.radius * scale, collision.height * scale);
            ...
            collision.localTransform.getOrigin().setValue(
                collision.position.x * scale, 
                collision.position.y * scale, 
                collision.position.z * scale
            );
